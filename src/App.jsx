...
export default function App() {
  ...
  async function loadSupabaseData() {
    ...
    setLoading(true);
    const [classResult, studentResult, unitResult, wordResult] = await Promise.all([
    ...
    const error = classResult.error || studentResult.error || unitResult.error || wordResult.error;
    if (error) {
  ...
  function recordCompletedAssignment(finalScore) {
    const now = new Date();
    ...
    setAssignmentRecords((previous) => {
      ...
      if (existing) return previous.map((record) => record.id === recordId ? { ...record, score: Math.max(record.score, finalScore), attempts: record.attempts + 1, completedAt } : record);
      return [{ id: recordId, classId: studentClassId, studentName, assignmentName, unitTitle: selectedLevel.title, score: finalScore, attempts: 1, completedWords: wordOrder.length, totalWords: wordOrder.length, status: '已完成', completedAt }, ...previous];
    });
  }
        ...
        <main className="main-grid">
          ...
          <section className="game-column">
            ...
            <Card className="game-card">
              <div className="game-header"><div><span>目前練習題庫</span><h2>{selectedLevel.title}</h2><p>本次任務需要完成 Unit 所有單字</p></div><div className="score-grid"><strong>{score}</strong><span>分數</span><strong>{streak}</strong><span>連勝</span><strong>{wrongCount}</strong><span>錯誤</span></div></div>
              <div className="game-body">
                ...
                {isUnitComplete && <div className="complete-box"><h3>🎉 Unit 完成！</h3><p>{studentName || '學生'} 已完成 {selectedLevel.title} 的全部 {wordOrder.length} 個單字。</p><p>本次分數：{score}</p><PrimaryButton onClick={startNewFullUnitRun}>重新開始完整 Unit 任務</PrimaryButton></div>}
              </div>
            </Card>
          </section>

          <aside className="side-column">
            <Card><div className="card-body"><h2>{selectedClass.name} 老師後台紀錄</h2><div className="stats-grid"><div><strong>{visibleAssignmentRecords.length}</strong><span>完成紀錄</span></div><div><strong>{averageScore}</strong><span>平均分數</span></div></div><div className="records-list">{visibleAssignmentRecords.length === 0 && <div className="notice">目前這個班級還沒有作業紀錄。</div>}{visibleAssignmentRecords.map((record) => <div key={record.id} className="record-card"><strong>{record.studentName}</strong><b>{record.score} 分</b><span>{record.unitTitle}</span><small>練習次數：{record.attempts}｜最近完成：{record.completedAt}</small></div>)}</div></div></Card>
          </aside>
        </main>
      </div>
    </div>
